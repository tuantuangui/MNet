                      ...
                      verbose = TRUE)
{
  ...
  if (!is(ig, "igraph")) {
    ...
    if (verbose) {
      ...
      message(
        sprintf(
          "First, generate a subgraph induced (via '%s' mode) by the annotation data (%s) ...",
          path.mode,
          as.character(now)
        ),
        appendLF = TRUE
  ...
  ind.length <- which(gs.length >= size.range[1] &
                        gs.length <= size.range[2])
  ...
  doFisherTest <- function(genes.group,
                           genes.term,
                           genes.universe) {
    ...
    cTab <- matrix(c(X, K - X, M - X, N - M - K + X),
                   nrow = 2,
                   dimnames = list(c("anno", "notAnno"), c("group", "notGroup")))
    
    if (0) {
      p.value <- ifelse(all(cTab == 0),
                        1,
                        stats::fisher.test(cTab, alternative = "greater")$p.value)
    } else{
      if (all(cTab == 0)) {
        p.value <- 1
      } else{
  ...
  doHypergeoTest <- function(genes.group,
                             genes.term,
                             genes.universe,
                             p.tail) {
    genes.hit <- intersect(genes.group, genes.term)
  ...
  doBinomialTest <- function(genes.group,
                             genes.term,
                             genes.universe,
                             p.tail) {
    genes.hit <- intersect(genes.group, genes.term)
  ...
  zscoreHyper <- function(genes.group,
                          genes.term,
                          genes.universe) {
    genes.hit <- intersect(genes.group, genes.term)
    # num of success in sampling
    ...
    if (1) {
      ...
      suppressWarnings(d <- stats::dhyper(x, m, n, k, log = TRUE) -
                         log(2))
      suppressWarnings(pupper <- stats::phyper(x, m, n, k, lower.tail =
                                                 FALSE, log.p = TRUE))
      suppressWarnings(plower <- stats::phyper(x - 1, m, n, k, lower.tail =
                                                 TRUE, log.p = TRUE))
      d[is.na(d)] <- -Inf
      ...
      if (any(up))
        z <- stats::qnorm(pmidupper, lower.tail = FALSE, log.p = TRUE)
      if (any(!up))
        z <- stats::qnorm(pmidlower, lower.tail = TRUE, log.p = TRUE)
    }
  ...
  if (verbose) {
    now <- Sys.time()
    message(sprintf(
      "Next, prepare enrichment analysis (%s) ...",
      as.character(now)
    ),
    appendLF = TRUE)
  }
  ...
  if (length(genes.group) == 0) {
    ...
    if (verbose) {
      ...
      message(
        sprintf(
          "\tThere are %d genes/SNPs of interest tested against %d genes/SNPs as the background (annotatable only? %s) (%s)",
          length(genes.group),
          length(genes.universe),
  ...
  subg <- dnet::dDAGinduce(g = subg,
                           nodes_query = terms,
                           path.mode = path.mode)
  set_info <- data.frame(
    id = V(subg)$term_id,
    name = V(subg)$term_name,
    distance = V(subg)$term_distance,
    namespace = V(subg)$term_namespace,
    row.names = V(subg)$name,
  ...
  if (ontology.algorithm == "none") {
    if (verbose) {
      ...
      message(
        sprintf(
          "Third, perform enrichment analysis using '%s' test (%s) ...",
          test,
          as.character(now)
        ),
        appendLF = TRUE
      ...
      if (is.null(which.distance)) {
        message(
          sprintf(
            "\tThere are %d terms being used, each restricted within [%s] annotations",
            length(terms),
            paste(size.range, collapse = ",")
          ),
          appendLF = TRUE
        )
      } else{
        message(
          sprintf(
            "\tThere are %d terms being used, each restricted within [%s] annotations and [%s] distance",
            length(terms),
            paste(size.range, collapse = ","),
            paste(which.distance, collapse = ",")
          ),
          appendLF = TRUE
        )
      }
    ...
    pvals <- sapply(terms, function(term) {
      ...
      p.value <- switch(
        ...
        hypergeo = doHypergeoTest(genes.group, genes.term, genes.universe, p.tail =
                                    p.tail),
        binomial = doBinomialTest(genes.group, genes.term, genes.universe, p.tail =
                                    p.tail)
      )
    })
  ...
  } else if (ontology.algorithm == "pc" ||
             ontology.algorithm == "elim" || ontology.algorithm == "lea") {
    if (verbose) {
      now <- Sys.time()
      message(
        sprintf(
          "Third, perform enrichment analysis based on '%s' test, and also using '%s' algorithm to respect ontology structure (%s) ...",
          test,
          ontology.algorithm,
          as.character(now)
        ),
    ...
    if (verbose) {
      message(sprintf("\tThere are %d terms being used", length(V(subg))),
              appendLF = TRUE)
    }
    
    level2node <- dnet::dDAGlevel(subg, level.mode = "longest_path", return.mode =
                                    "level2node")
    ...
    if (ontology.algorithm == "pc") {
      for (i in nLevels:2) {
        ...
        for (currNode in currNodes) {
          ...
          neighs.in <- igraph::neighborhood(subg,
                                            order = 1,
                                            nodes = currNode,
                                            mode = "in")
          ...
          pvalue_relative <- switch(
            ...
            hypergeo = doHypergeoTest(
              genes.group.parent,
              genes.term.parent,
              genes.parent,
              p.tail = p.tail
            ),
            binomial = doBinomialTest(
              genes.group.parent,
              genes.term.parent,
              genes.parent,
              p.tail = p.tail
        ...
        if (verbose) {
          message(sprintf(
            "\tAt level %d, there are %d nodes/terms",
            i,
            length(currNodes),
            appendLF = TRUE
          ))
      ...
      if (is.null(elim.pvalue) ||
          is.na(elim.pvalue) || elim.pvalue > 1 || elim.pvalue < 0) {
        elim.pvalue <- 1e-2
      }
      ...
      for (i in nLevels:1) {
        ...
        for (currNode in currNodes) {
          ...
          if (exists(currNode, envir = ancNode2gene.Hash, mode =
                     "numeric")) {
          ...
          if (pvalue < pval.cutoff) {
            ...
            dag.ancestors <- dnet::dDAGinduce(subg, currNode, path.mode =
                                                "all_paths")
        ...
        if (verbose) {
          ...
          num.elimgenes <- length(unique(unlist(as.list(
            ancNode2gene.Hash
          ))))
          message(
            sprintf(
              "\tAt level %d, there are %d nodes/terms: up to %d significant nodes, %d ancestral nodes changed (%d genes/SNPs eliminated)",
              i,
              length(currNodes),
              num.signodes,
              num.ancnodes,
      ...
      if (is.null(lea.depth) ||
          is.na(lea.depth) || lea.depth < 0) {
        lea.depth <- 2
      }
      ...
      for (i in nLevels:1) {
        ...
        for (currNode in currNodes) {
          ...
          neighs.out <- igraph::neighborhood(subg,
                                             order = depth.cutoff,
                                             nodes = currNode,
                                             mode = "out")
        ...
        if (verbose) {
          message(
            sprintf(
              "\tAt level %d, there are %d nodes/terms and %d being recalculated",
              i,
              length(currNodes),
              num.recalculate
            ),
  ...
  flag_filter <- sapply(overlaps, function(x)
    ifelse(length(x) >= min.overlap, TRUE, FALSE))
  ...
  if (verbose) {
    ...
    message(
      sprintf(
        "Last, adjust the p-values for %d terms (with %d minimum overlaps) using the %s method (%s) ...",
        length(pvals),
        min.overlap,
        p.adjust.method,
        as.character(now)
