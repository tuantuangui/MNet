...
#[which(!metabolites_name %in% refmet_database$`Input name`)]
  ...
  if (length(metabolites_to_standardized_temp)!=0){
    ...
    for (i in seq(1,length(metabolites_to_standardized_temp))) {
      ...
      if (length(grep("AC$",name1))>0 & length(grep("^C",name1))>0) {
        name1_new <- gsub("^C","CAR+",name1)
        name <- gsub(" AC$","",name1_new)
      }

      if (length(grep(",OH FA$",name1))>0 & length(grep("^C",name1))>0) {
        name=gsub(",OH FA$","-OH",name1)
      }

      if (length(grep(" FA$",name1))>0 & length(grep("^C",name1))>0 & length(grep("OH",name1))==0) {
      ...
      url <- paste("https://www.metabolomicsworkbench.org/rest/refmet/match/",name,"/name/txt",sep="")
      ...
      if (length(aa)==0) {
        aa <- data.frame(refmet_name=NA,formula=NA,exactmass=NA,super_class=NA,main_class=NA,sub_class=NA,class_index=NA)
