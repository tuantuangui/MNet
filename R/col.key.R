col.key <-
  function(discrete = FALSE,
    ...
    if (!discrete) {
      ...
      graphics::text(
        x = seq(x, x + bins * xs, length = length(label)),
        y = rep(y - ys, length(label)),
        label = label,
        cex = cex
      )
    } else{
      ...
      graphics::text(
        x = seq(x, x + bins * xs, length = length(label)) + c(xs, -xs) / 2,
        y = rep(y - ys, length(label)),
        label = label,
        cex = cex
      )
    ...
    for (i in 1:(bins + 1))
      graphics::lines(rep(ckx[i], 2), cky, lwd = lwd)
