...
PathwayAnalysis <- function(name,out="Extended",p_cutoff=0.05,noverlap_cutoff=0,test = c("hypergeo","fisher","binomial")) {
  ...
  result_1$`Pathway Category` <- factor(result_1$`Pathway Category`,levels=unique(kegg_pathway_uniq$`Pathway Category`))

  p1 <- ggplot(result_1,aes(name,-log10(pvalue)))+
    geom_bar(stat="identity",aes(fill=`Pathway Category`))+
    scale_fill_manual(values=RColorBrewer::brewer.pal(11, "Set3"),
                      breaks=unique(kegg_pathway_uniq$`Pathway Category`))+
    coord_flip()+
    theme_bw()+
    labs(x=NULL)
  result$p_barplot <- p1
