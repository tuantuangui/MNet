#' @title Heatmap of differential metabolites
#'
...
pHeatmap <- function(object,
  ...
  color = c(
    grDevices::colorRampPalette(colors = c("blue", "white"))(length(seq(
      round(min(object_norm), 1), -0.01, by = 0.01
    ))),
    grDevices::colorRampPalette(colors = c("white", "red"))(length(seq(
      0, round(max(object_norm), 1), by = 0.01
    )))
  )
  ...
  annotation_colors = list(group = c(tumor = tumor_color, normal = normal_color))
  p <- pheatmap::pheatmap(
    object_norm,
    color = color,
