#' @title The visualization of ESEA in a pathway
#'
...
pESEA <- function(pathway_name,
  ...
  g1 <- ggplot2::ggplot(toPlot, aes(x = x, y = y)) +
    ...
    ggplot2::scale_x_continuous(expand = c(0, 0), limits = c(0, length(statsAdj) +
                                                               1)) +
  ...
  p1 <- cowplot::plot_grid(
    ...
    scale = 1,
    align = "v",
    labels = paste0(pathway_name, " pval=", pval, ";NES=", nes),
    label_size = 8
  )
  return(p1)
}
