#' @title Fold change, p value and vip of the data
#'
...
DM <- function(mydata, group) {
  ...
  for (i in seq(1, nrow(mydata_norm))) {
    ...
    if (length(unique(group1_var)) == 1 &&
        length(unique(group2_var)) == 1 &&
        as.character(unique(group1_var)) == as.character(unique(group2_var))) {
      p_value_t[i] <- 1
      p_value_wilcox[i] <- 1
      print(i)
    } else if (length(unique(group1_var)) == 1 &&
               length(unique(group2_var)) == 1 &&
               unique(group1_var) != unique(group2_var)) {
      p_value_t[i] <- 0
      p_value_wilcox[i] <- 0
      print(i)
    } else {
  ...
  p_value_result <- data.frame(
    t_value = p_value_t,
    fdr_t = fdr_t,
    wilcox_value = p_value_wilcox,
    fdr_wilcox = fdr_wilcox
  )
