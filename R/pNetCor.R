...
pNetCor <- function(dat_meta,dat_gene,cor_method="pearson",cor_threshold=0.9,p_threshold=0.01) {
  ...
  result <- result_cor_melt %>%
    ...
    dplyr::mutate(type1=ifelse(name1 %in% gene_name,"gene",
                               ifelse(name1 %in% metabolite_name,"metabolite","other"))) %>%
    dplyr::mutate(type2=ifelse(name2 %in% gene_name,"gene",
  ...
  p <- plot(network,vertex.color=name$colors,vertex.shape=my_shape,vertex.size=9,
